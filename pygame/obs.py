...
def draw_food(food):
    color = (255,255,0) if food['value'] == 1 else RED if food['value'] == 2 else (0, 200, 255)
    pygame.draw.rect(screen, color, (*food['pos'], BLOCK_SIZE, BLOCK_SIZE))

def draw_obs(o):
    pygame.draw.rect(screen, WHITE, (o[0], o[1], BLOCK_SIZE, BLOCK_SIZE))
...
def game():
    ...
    o = obs(snake)  # Генерация препятствия
    score = 0  # Очки
    level = 1  # Уровень
    ...
    while running:
        ...
        if (head_x, head_y) == food['pos']:
            score += food['value']  # Увеличиваем счет
