import pygame
import pygame_list as pl
from pygame import mixer

pygame.init()
pygame.mixer.set_num_channels(50)

font = pygame.font.Font('pygame/Terserah.ttf', 48)
medium_font = pygame.font.Font('pygame/Terserah.ttf', 28)
small_font = pygame.font.Font('pygame/Terserah.ttf', 16)
real_small_font = pygame.font.Font('pygame/Terserah.ttf', 10)
...
for i in range(len(white_notes)):
    white_sounds.append(mixer.Sound(f'pygame/notes/{white_notes[i]}.wav'))

for i in range(len(black_notes)):
    black_sounds.append(mixer.Sound(f'pygame/notes/{black_notes[i]}.wav'))
...
def draw_piano(whites, blacks):
    white_rects = []
    for i in range(52):
        rect = pygame.draw.rect(screen, 'white', [i * 35, h - 300, 35, 300], 0, 2)
        white_rects.append(rect)
        pygame.draw.rect(screen, 'black', [i * 35, h - 300, 35, 300], 2, 2)
        key_label = small_font.render(pl.white_notes[i], True, 'black')
        screen.blit(key_label, (i * 35 + 3, h - 20))
    skip_count = 0
    last_skip = 2
    skip_track = 2
    black_rects = []
    for i in range(36):
        rect = pygame.draw.rect(screen, 'black', [23 + (i * 35) + (skip_count * 35), h - 300, 24, 200], 0, 2)
        for q in range(len(blacks)):
            if blacks[q][0] == i:
                if blacks[q][1] > 0:
                    pygame.draw.rect(screen, 'green', [23 + (i * 35) + (skip_count * 35), h - 300, 24, 200], 2, 2)
                    blacks[q][1] -= 1

        key_label = real_small_font.render(pl.black_labels[i], True, 'white')
        screen.blit(key_label, (25 + (i * 35) + (skip_count * 35), h - 120))
    ...
    for i in range(len(whites)):
        if whites[i][1] > 0:
            j = whites[i][0]
            pygame.draw.rect(screen, 'green', [j * 35, h - 100, 35, 100], 2, 2)
            whites[i][1] -= 1
...
def draw_title_bar():
    ...
    img = pygame.transform.scale(pygame.image.load('pygame/notes/logo.png'), [150, 150])
...
while run:
    ...
    clock.tick(60)
    screen.fill("gray")
    white_keys, black_keys, active_whites, active_balcks =draw_piano(active_whites, active_blacks)
